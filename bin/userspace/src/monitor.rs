...
fn load_ebpf() -> anyhow::Result<Ebpf> {
    let program_data = include_bytes_aligned!(
        concat!("../../../target/bpfel-unknown-none/", env!("PROFILE"), "/ebpf")
    );

    Ok(Ebpf::load(program_data)?)
}

fn attach_tracepoint(bpf: &mut Ebpf, category: &str, name: &str) -> anyhow::Result<TracePointLinkId> {
...
fn umount_module_files(pid: i32) {
    ...
    let res: anyhow::Result<()> = try {
        let link: OwnedFd = File::open(format!("/proc/{}/ns/mnt", pid)).unwrap().into();
        debug!("switching into mount namespace: {pid}");
        thread::move_into_link_name_space(link.as_fd(), None).unwrap();

        let proc = Process::myself().unwrap();

        let mounts: Vec<MountInfo> = proc.mountinfo().unwrap().into_iter().collect();
        ...
        for mount in mounts.into_iter().rev() {
            let mp = CString::new(mount.to_string_lossy().to_string()).unwrap();
            debug!("[{pid}] umount: {}", mp.as_str().unwrap());
...
pub async fn main(filter: Option<&str>) -> anyhow::Result<()> {
    bump_rlimit();
    ...
    loop {
        ...
        let res: anyhow::Result<()> = try {
            let buffer: [u8; size_of::<EbpfEvent>()] = (*entry.unwrap()).try_into().unwrap();
            ...
            match event {
                ...
                EbpfEvent::RequireUprobeAttach(pid) => {
                    ...
                    let link_id = uprobe.attach(None, func_addr, uprobe_lib, Some(pid)).unwrap();
                ...
                EbpfEvent::RequireInject(pid, return_addr) => {
                    ...
                    let config = BridgeConfig {
                        library: "/data/adb/modules/zloadersu/lib/libloader.so",
