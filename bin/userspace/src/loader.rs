...
use public::zygote::SpecializeArgs;
...
impl Tracee {
    ...
    #[cfg(target_arch = "x86_64")]
    fn regs(&self) -> anyhow::Result<Registers> {
    ...
    #[cfg(target_arch = "x86_64")]
    fn set_regs(&self, regs: &Registers) -> anyhow::Result<()> {
        Errno::result(unsafe {
    ...
    fn peek(&self, addr: usize) -> anyhow::Result<u64> {
        Ok(ptrace::read(self.pid, addr as _)? as u64)
    }

    fn poke(&self, addr: usize, value: u64) -> anyhow::Result<()> {
        unsafe {
    ...
    #[cfg(target_arch = "x86_64")]
    fn arg(&self, regs: &Registers, n: usize) -> anyhow::Result<u64> {
        let arg = if n < 6 {
            regs.arg(n)
        } else {
            let n = n - 6;
    ...
    fn set_arg(&self, regs: &mut Registers, n: usize, value: u64) -> anyhow::Result<()> {
    ...
    #[cfg(target_arch = "x86_64")]
    fn set_return_addr(&self, regs: &mut Registers, addr: usize, alloc: bool) -> anyhow::Result<()> {
    ...
    #[cfg(target_arch = "aarch64")]
    fn set_return_addr(&self, regs: &mut Registers, addr: usize, _alloc: bool) -> anyhow::Result<()> {
        regs.0.regs[30] = addr as _;
        Ok(())
    }

    fn alloc(&self, regs: &mut Registers, data: &[u8]) -> anyhow::Result<usize> {
    ...
    fn wait(&self) -> anyhow::Result<WaitStatus> {
        loop {
    ...
    #[allow(dead_code)]
    fn debug_call(&self) -> anyhow::Result<()> {
        let pid = self.pid;
    ...
    fn call(&self, regs: &Registers, func: usize, args: &[u64], return_addr: usize) -> anyhow::Result<u64> {
        let retval: anyhow::Result<u64> = try {
...
impl<'a> TraceeWrapper<'a> {
    fn new(tracee: &'a Tracee) -> anyhow::Result<Self> {
    ...
    fn update_maps(&mut self) -> anyhow::Result<()> {
    ...
    fn read_string(&self, addr: usize) -> anyhow::Result<String> {
    ...
    fn read_jstring(&self, jnienv: usize, jstring: usize) -> anyhow::Result<String> {
        let tracee = self.tracee;
    ...
    fn find_module(&self, name: &str) -> anyhow::Result<&(PathBuf, usize)> {
        self.modules.get(name)
            .context(format!("[{}] failed to find module {name}", self.pid()))
    }

    fn find_symbol_addr(&self, lib: &str, func: &str) -> anyhow::Result<usize> {
...
fn check_process(wrapper: &TraceeWrapper, args: &[u64], filter: Option<&FilterFn>) -> anyhow::Result<bool> {
...
fn remote_dlopen(wrapper: &mut TraceeWrapper, bridge: &str) -> anyhow::Result<()> {
    ...
    fn dlerror(wrapper: &TraceeWrapper, func: usize) -> anyhow::Result<()> {
...
fn unmap_uprobes(wrapper: &TraceeWrapper) -> anyhow::Result<()> {
...
fn load_bridge(tracee: &Tracee, config: &BridgeConfig) -> anyhow::Result<()> {
...
pub fn handle_proc(pid: i32, config: &BridgeConfig) -> anyhow::Result<()> {
