...
impl ArgCounter {
    ...
    pub fn count(sym: &str) -> anyhow::Result<usize> {
...
pub fn resolve<P : AsRef<Path>>(library: P, name: &str) -> anyhow::Result<usize> {
...
pub fn resolve_for_uprobe<P : AsRef<Path>>(library: P, prefix: &str) -> anyhow::Result<(String, u64)> {
    let data = fs::read(library)?;

    fn internal(data: &[u8], prefix: &str, mirror: Option<&File>) -> anyhow::Result<(String, u64)> {
